...
The explorer retains its three header rows and stores footer fragments by semantic message ID and source-text byte offset. Reflow restores a footer cursor to the same message and clamps its offset if the text shrinks; a removed message leaves the cursor within the new footer. Progress and stopped-review messages are not footer fragments: they are pinned to the bottom of the pane and are not reachable by the cursor, so the stored fragments cover the saved-line summary, the unsaved-buffer warning and the selected file's details. Directory rendering caches the resolved open/closed symbols by value. Display widths govern wrapping and alignment, while highlight and cursor positions use byte offsets.
...
[status.lua](../lua/diffreel/status.lua) owns the nonfocusable overlay pinned to the explorer's bottom rows. It carries the waiting and stopped-review messages, computes its row in editor coordinates from the explorer window's first row, winbar and text height, reserves an equal `scrolloff` so the cursor never descends behind it, and closes itself when the panel hides, the layout changes or the review ends. When those messages would need more than half the pane, the overlay is skipped and the whole block is written into the buffer instead, so no line disappears without a way to scroll to it; those rows are plain lines, not footer fragments, so they hold no cursor either. The rows the overlay covers are blank reservations, so nothing the buffer carries is hidden behind it.
