...
local ok, err = xpcall(function()
  ...
  assert(hl("LineAdd").bg == 0x112030, "Line backgrounds did not blend with the theme")
  assert(hl("ExplorerTitle").link == "DiffreelTitle")
  ...
  assert(hl("LineAdd").bg == highlights.defaults().DiffreelLineAdd.bg)
  ...
  vim.api.nvim_set_hl(0, "Normal", { fg = 0xeeeeee })
  local transparent = highlights.defaults()
  assert(transparent.DiffreelExplorerNormal.link == "Normal", "A transparent theme received an opaque panel")
  assert(transparent.DiffreelDiffWinBar.link == "WinBar")
  assert(transparent.DiffreelLineAdd.bg == 0x334455)
end, debug.traceback)
plugin.shutdown()
if not ok then
  io.stderr:write(err .. "\n")
