...
// A frame can arrive before the plugin paints into it, so a single read races the redraw.
async function eventually<T>(nvim: Nvim, check: () => T, timeout = 10) {
  ...
  for (;;) {
    ...
    try {
      ...
      if (now() >= deadline) throw flushed ?? error;
      await sleep(0.025);
    }
  }
}
