...
local ok, err = xpcall(function()
  ...
  assert(highlight(view.left_win, "DiffChange").bg == 0x503040, "Old lines do not use the deletion background")
  assert(highlight(view.right_win, "DiffChange").bg == 0x123040, "New lines do not use the insertion background")
  ...
  assert(highlight(view.right_win, "DiffChange").bg == 0x123040)
  vim.api.nvim_set_hl(0, "DiffAdd", { bg = 0x203050 })
  vim.api.nvim_exec_autocmds("ColorScheme", { pattern = "test" })
  assert(highlight(view.right_win, "DiffChange").bg == 0x203050, "Colors did not follow the theme")
